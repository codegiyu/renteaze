...
const SelectWithPicture: React.FC<Select> = ({
  ...
  const [selectedPic, setSelectedPic] = useState<string>(
    optionsArray.find((item) => item.value === value)?.image || ""
  );
  ...
  const handleOptionsClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (select && newValue) {
      ...
      const newImage =
        optionsArray.find((item) => item.value == newValue)?.image || "";
  ...
  return (
    <label
      className={`select-wrap ${errMsg ? "error" : ""} ${
    ...
    >
      ...
      <div className="select" tabIndex={1}>
        <div
          className={`header picture-select ${borders ? "has-borders" : ""}`}
          ref={selectRef}
          onClick={handleClick}
        >
          <Image
          ...
          <span className={`select-text ${value ? "" : "placeholder"}`}>
            {optionsArray.find(
              (item: SelectOptionProps) => item.value === value
            )?.text || placeholder}
          </span>
          <Image src={caret} alt="" width={24} height={24} />
          <select
            name={name}
            className=""
            value={value}
